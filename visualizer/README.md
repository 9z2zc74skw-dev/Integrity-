...
2. **Drag** a FedSig SKU onto the vehicle, or **click** a SKU to place it on the **current** view (the camera does not jump). Mirror and other pairable side lights place **one** node on that view — Left click = left, Right click = right. No auto-pair. Visor ILS still places both shrouds; the bumper-round SKU still places its named set.
